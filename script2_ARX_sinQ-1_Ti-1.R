...

{
  for (j in 0:obs_anteriores){
    ...
    for (i in 1:length(variables_regresion_input)){
      if (!(variables_regresion_input[i] %in% c("energia_agua_refrigerada_1", "temperatura_interior_1"))) {
        formula <- paste(formula, " ", variables_regresion_input[i], sep="") 
        if (i < length(variables_regresion_input)) {
          formula <- paste(formula, " +", sep="")
        }
    ...
    if (j > 0) {
      for (i in 1:j) {
        nombre_var <- paste(regresion_output, "_", i, sep="")
        if (!(nombre_var %in% c("energia_agua_refrigerada_1", "temperatura_interior_1"))) {
          formula <- paste(formula, " + ", nombre_var, sep="")
        }
        ...
        for (k in 1:length(nombre_var)) {
          if (!(nombre_var[k] %in% c("energia_agua_refrigerada_1", "temperatura_interior_1"))) {
...
write.csv2(dataframe_resultados, 
           paste(wd, "/output/dataframe_resultados_1_sinQ-1_Ti-1.csv", sep=""))
